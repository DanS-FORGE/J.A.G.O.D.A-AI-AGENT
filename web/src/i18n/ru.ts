...
export const ru: Translations = {
  ...
  status: {
    ...
    stopped: "Остановлено",
    updateHermes: "Обновить J.A.G.O.D.A",
    updatingHermes: "Обновление J.A.G.O.D.A…",
  ...
  pluginsPage: {
    ...
    headline:
      "Поиск, установка, включение и обновление плагинов J.A.G.O.D.A (аналог `hermes plugins`).",
  ...
  env: {
    ...
    customHint: "Произвольные переменные окружения, сохранённые в вашем .env, которые J.A.G.O.D.A не распознаёт. Используйте их для внедрения переменных окружения для навыков, серверов MCP или собственных инструментов.",
  ...
  achievements: {
    hero: {
      ...
      subtitle:
        "Коллекционные значки J.A.G.O.D.A, полученные на основе реальной истории сессий. Известные, но ещё не полученные достижения отображаются как «Обнаруженные»; «Секретные» достижения остаются скрытыми до появления первого подходящего поведения.",
      scan_subtitle:
        "Анализ истории сессий J.A.G.O.D.A. Первое сканирование может занять 5–10 секунд при большой истории.",
    },
    actions: {
    ...
    stats: {
      ...
      latest: "Последнее",
      latest_hint_empty: "запускайте J.A.G.O.D.A чаще",
      none_yet: "Пока нет",
    },
    state: {
      unlocked: "Разблокировано",
    ...
    guide: {
      ...
      secret_body:
        "Секретные достижения скрывают свой точный триггер. Как только Jagoda обнаруживает связанный сигнал, карточка становится «Обнаруженной» и показывает требование.",
      scan_status_header: "Статус сканирования",
      scan_status_body:
        "J.A.G.O.D.A сканирует локальную историю один раз, затем карточки появятся автоматически. Если это занимает несколько секунд — ничего не зависло.",
