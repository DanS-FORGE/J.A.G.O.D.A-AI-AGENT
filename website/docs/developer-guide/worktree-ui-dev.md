...
Neither is read by J.A.G.O.D.A itself — they're private to these helpers. The variables J.A.G.O.D.A *does* read are covered in [Environment Variables](../reference/environment-variables.md).
...
htui() {
  local root
  root="$(_hermes_root)" || { echo "htui: not in a J.A.G.O.D.A checkout" >&2; return 1; }
...
`HERMES_TUI_DIR` points J.A.G.O.D.A at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `hermes --tui --dev` exits with an error. Run `unset HERMES_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_hermes_root)" || { echo "hgui: not in a J.A.G.O.D.A checkout" >&2; return 1; }
...
```bash
# The enclosing worktree, verified as a real J.A.G.O.D.A checkout.
_hermes_root() {
  local root
...
- [Environment Variables](../reference/environment-variables.md) — every `HERMES_*` variable J.A.G.O.D.A reads
