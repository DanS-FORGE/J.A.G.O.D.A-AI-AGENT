...
description: "How J.A.G.O.D.A builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

J.A.G.O.D.A deliberately separates:
...
You are J.A.G.O.D.A, an AI assistant created by Nous Research.
...
The platform hint (Layer 10 above) is the per-surface guidance J.A.G.O.D.A
...
```
You are J.A.G.O.D.A, built by Nous Research. Be direct: match the length
...
|----------|-------|-------------|-------|
| 1 | `.hermes.md`, `HERMES.md` | CWD up to git root | J.A.G.O.D.A-native project config |
...
`pre_llm_call` plugin context also lands in this API-call-time path: it is appended to the current turn's **user message**, not written into the cached system prompt. When multiple plugins return context, J.A.G.O.D.A concatenates those context blocks (see [Hooks → `pre_llm_call`](../user-guide/features/hooks.md#pre_llm_call)).
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs J.A.G.O.D.A already loads, rather than editing Python templates in place.
...
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking J.A.G.O.D.A.
...
- if you want to change how J.A.G.O.D.A assembles prompts for everyone, change Python and treat it as a code contribution
