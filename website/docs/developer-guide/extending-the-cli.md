...
description: "Build wrapper CLIs that extend the J.A.G.O.D.A TUI with custom widgets, keybindings, and layout changes"
---

# Extending the CLI

J.A.G.O.D.A exposes protected extension hooks on `HermesCLI` so wrapper CLIs can add widgets, keybindings, and layout customizations without overriding the 1000+ line `run()` method. This keeps your extension decoupled from internal changes.
...
```python
#!/usr/bin/env python3
"""my_cli.py — Example wrapper CLI that extends J.A.G.O.D.A."""
...
Called after J.A.G.O.D.A registers its own keybindings and before the layout is built. Add your keybindings to `kb`.
