...
description: "How J.A.G.O.D.A detects and responds to native JS dialogs and interacts with cross-origin iframes via a persistent CDP connection."
---

# Browser CDP Supervisor

The CDP supervisor closes two long-standing gaps in J.A.G.O.D.A's browser tooling:
...
One `asyncio.Task` running in a background daemon thread per J.A.G.O.D.A `task_id`.
