...
title: "Build a J.A.G.O.D.A Plugin"
description: "Step-by-step guide to building a complete J.A.G.O.D.A plugin with tools, hooks, data files, and skills"
---

# Build a J.A.G.O.D.A Plugin

This guide walks through building a complete J.A.G.O.D.A plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
J.A.G.O.D.A has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
J.A.G.O.D.A can also install and load directory packages that target the Agent
Plugins v1.0.0 format. This is a compatibility adapter for the portable
components J.A.G.O.D.A already owns. It does not replace native `plugin.yaml` plus
...
J.A.G.O.D.A validates `plugin.json`, Agent Skills frontmatter, fixed component
...
profile-scoped writable directory managed by J.A.G.O.D.A.
...
Portable `streamable-http` entries are routed through J.A.G.O.D.A's existing native
...
same full-trust posture as other installed J.A.G.O.D.A plugins.
...
records it as Published. J.A.G.O.D.A keys behavior on the canonical v1.0.0 schema
...
not by one global plugin API number. J.A.G.O.D.A does not expose a
...
continue to work after a normal J.A.G.O.D.A upgrade.
...
- **Hook payloads are keyword payloads.** New hook data is added as keyword
  ...
  J.A.G.O.D.A inspects callback signatures: a legacy callback receives the fields it
...
- **Manifests are open to additions.** Unknown `plugin.yaml` fields are ignored.
  Older J.A.G.O.D.A releases can therefore load a plugin whose manifest contains
...
J.A.G.O.D.A enforces this contract with frozen external-plugin fixtures discovered
...
registry used by J.A.G.O.D.A itself. It reports invalid hook names, callbacks that do
...
This tells J.A.G.O.D.A: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
J.A.G.O.D.A reads these from installed metadata without importing your code, so
...
this J.A.G.O.D.A understands still loads with a warning.
...
| `python_dependencies` | list of str | Declared pip requirements (e.g. `"requests>=2.0,<3"`). **Declaration seam only** — J.A.G.O.D.A validates them, and `hermes plugins install` / `hermes plugins doctor` surface missing ones with a `pip install` hint, but J.A.G.O.D.A **never auto-installs** them. Pin upper bounds. |
...
arbitrary packages into J.A.G.O.D.A's shared venv is a conflict and supply-chain
...
4. **Accept `**kwargs`:** J.A.G.O.D.A may pass additional context in the future.
...
- If this function crashes, the plugin is disabled but J.A.G.O.D.A continues fine
...
Use plugin-relative config keys for user-visible behavior. J.A.G.O.D.A resolves them
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing J.A.G.O.D.A into installing arbitrary packages — only specs J.A.G.O.D.A itself ships are eligible |
...
does not ship with J.A.G.O.D.A core (user, project, or pip source),
...
loader, the plugin is disabled and J.A.G.O.D.A continues. The gate exists
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), J.A.G.O.D.A injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.
...
- **The system prompt is J.A.G.O.D.A's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
| `llm_request` | `request`, `original_request`, context kwargs | Return `{"request": {...}}` to replace the effective provider kwargs before J.A.G.O.D.A sends them. |
...
- Unknown kinds register with a warning instead of failing, so a plugin written against a newer J.A.G.O.D.A still loads on an older one.
...
For running a full `hermes <subcommand>` (e.g. `hermes kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "hermes kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive J.A.G.O.D.A from a hook.
...
J.A.G.O.D.A has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.hermes/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
J.A.G.O.D.A also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into J.A.G.O.D.A without any Python plugin. Declare them in `~/.hermes/config.yaml`:
...
J.A.G.O.D.A connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).
...
```python
# Wrong — will break if J.A.G.O.D.A passes extra context
def handler(args):
    ...
