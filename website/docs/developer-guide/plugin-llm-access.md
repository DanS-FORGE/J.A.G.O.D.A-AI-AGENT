...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept J.A.G.O.D.A already has applies. The plugin never sees a
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — J.A.G.O.D.A provider id (e.g. "openrouter")
...
using J.A.G.O.D.A built-in auxiliary tasks; it does not permit another plugin's
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different J.A.G.O.D.A provider
        # (must be one J.A.G.O.D.A already knows about — same names as
        ...
        # plugin sends — J.A.G.O.D.A does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through J.A.G.O.D.A's usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing J.A.G.O.D.A subsystem:
