...
Plugins can launch and supervise fresh J.A.G.O.D.A child sessions without importing
...
active agent turn fails closed with `No active J.A.G.O.D.A parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until J.A.G.O.D.A can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; J.A.G.O.D.A's existing unsafe-tool block remains enforced.
