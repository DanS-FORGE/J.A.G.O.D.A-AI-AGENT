...
J.A.G.O.D.A can show an animated **pet** — a small mascot sprite that reacts to what
...
In the terminal (CLI/TUI), J.A.G.O.D.A renders the sprite at full fidelity when your
...
Beyond installing pre-made pets from the gallery, J.A.G.O.D.A can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.
...
always-on-top desktop window. Out there it stays visible while J.A.G.O.D.A is
...
- The petdex npm CLI installs to `~/.codex/pets`; J.A.G.O.D.A uses its own
