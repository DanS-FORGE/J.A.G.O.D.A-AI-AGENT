...
J.A.G.O.D.A includes a full browser automation toolset with multiple backend options:
...
- **Lightpanda local engine** via [Lightpanda](https://lightpanda.io) — a headless browser built from scratch in Zig for machines; instant start up, 16x lower memory and 9x faster than Chrome. Works in Browser Use mode (J.A.G.O.D.A spawns it, no Chromium or Node needed) and with the built-in tools (automatic Chrome fallback for actions it doesn't support yet)
...
**This is the default browser mode**: when `browser.backend` is unset and the `browser-use` CLI is runnable (installed, or available through `uvx`), the agent gets the single `browser_exec` tool. If the CLI can't run, J.A.G.O.D.A falls back to the built-in browser tools automatically.
...
When a cloud provider is configured, J.A.G.O.D.A auto-spawns a **local Chromium sidecar**
...
When enabled, J.A.G.O.D.A copies your default browser's **active** profile — the one
...
When you turn the toggle back off, J.A.G.O.D.A deletes the snapshot store
...
an exclusive (deny-all) lock, so J.A.G.O.D.A cannot copy them while the browser is
...
Set `browser.real_profile_autoclose: true` to let J.A.G.O.D.A **offer to close the
browser for you** when it's holding the profile. Even with this on, J.A.G.O.D.A never
...
(e.g. a background/tray instance relaunched), J.A.G.O.D.A stays blocked and tells you
...
Then fully restart J.A.G.O.D.A so the new config is picked up.

:::warning Nested path matters
J.A.G.O.D.A reads `browser.camofox.managed_persistence`, **not** a top-level `managed_persistence`. A common mistake is writing:

```yaml
# ❌ Wrong — J.A.G.O.D.A ignores this
managed_persistence: true
```

If the flag is placed at the wrong path, J.A.G.O.D.A silently falls back to a random ephemeral `userId` and your login state will be lost on every session.
:::

##### What J.A.G.O.D.A does
...
- Scopes the `userId` to the active J.A.G.O.D.A profile, so different J.A.G.O.D.A profiles get different browser profiles (profile isolation).

##### What J.A.G.O.D.A does not do
- It does not force persistence on the Camofox server. J.A.G.O.D.A only sends a stable `userId`; the server must honor it by mapping that `userId` to a persistent Firefox profile directory.
- If your Camofox server build treats every request as ephemeral (e.g. always calls `browser.newContext()` without loading a stored profile), J.A.G.O.D.A cannot make those sessions persist. Make sure you are running a Camofox build that implements userId-based profile persistence.

##### Verify it's working

1. Start J.A.G.O.D.A and your Camofox server.
...
If step 5 logs you out, the Camofox server isn't honoring the stable `userId`. Double-check your config path, confirm you fully restarted J.A.G.O.D.A after editing `config.yaml`, and verify your Camofox server version supports persistent per-user profiles.

##### Where state lives

J.A.G.O.D.A derives the stable `userId` from the profile-scoped directory `~/.hermes/browser_auth/camofox/` (or the equivalent under `$HERMES_HOME` for non-default profiles). The actual browser profile data lives on the Camofox server side, keyed by that `userId`. To fully reset a persistent profile, clear it on the Camofox server and remove the corresponding J.A.G.O.D.A profile's state directory.

#### Externally managed Camofox sessions

When another app drives the visible Camofox browser (a desktop assistant, a custom integration, another agent), configure J.A.G.O.D.A to operate inside that same identity instead of spawning its own isolated profile.

Three knobs control the behavior:

| Setting | Env var | Effect |
|---------|---------|--------|
| `browser.camofox.user_id` | `CAMOFOX_USER_ID` | Camofox `userId` J.A.G.O.D.A uses when creating tabs. Setting this opts the session into "externally managed" mode. |
...
| `browser.camofox.adopt_existing_tab` | `CAMOFOX_ADOPT_EXISTING_TAB` | When true, J.A.G.O.D.A calls `GET /tabs?userId=<user_id>` on first use and reuses an existing tab before creating a new one. |
...
- J.A.G.O.D.A skips destructive cleanup at task end (same as `managed_persistence: true`). The other app's tab/cookies/profile survive.
- J.A.G.O.D.A does **not** call `DELETE /sessions/<user_id>` — that endpoint wipes all user data, so it would nuke the external app's session if it fired.

**How tab adoption works (when `adopt_existing_tab: true`):**

1. On the first browser tool call after a process start, J.A.G.O.D.A issues `GET /tabs?userId=<user_id>` (5-second timeout).
2. If any tab in the response has `listItemId == session_key`, J.A.G.O.D.A adopts the most recently created one in that group.
3. Otherwise, J.A.G.O.D.A adopts the most recently created tab for the user (any `listItemId`).
4. If no tabs exist or the request fails, J.A.G.O.D.A falls back to creating a new tab on the next operation.

Adoption only fires until `tab_id` is populated for the session. If the external app closes the adopted tab mid-run, the next browser tool call will surface a Camofox error — J.A.G.O.D.A does not re-poll for a fresh tab on every call.

**Picking `session_key`:** if you want J.A.G.O.D.A to reliably attach to a *specific* existing tab, set `session_key` to the `listItemId` the external app used when creating it. If you leave `session_key` unset and only set `user_id`, J.A.G.O.D.A generates a per-task `session_key` (`task_<id>`) — J.A.G.O.D.A will share cookies and the profile with the external app, but will open its own tab alongside instead of reusing one.

**Concurrency note:** the external app and J.A.G.O.D.A can drive the same Camofox `userId` simultaneously, but Camofox does not coordinate per-tab focus between clients. Coordinate ownership at the application layer (e.g. the external app pauses while J.A.G.O.D.A runs).

#### VNC live view

When Camofox runs in headed mode (with a visible browser window), it exposes a VNC port in its health check response. J.A.G.O.D.A automatically discovers this and includes the VNC URL in navigation responses, so the agent can share a link for you to watch the browser live.
...
- **Browser Use mode (the default).** J.A.G.O.D.A launches `lightpanda serve --host 127.0.0.1 --port <free>` itself — one process per `browser_exec` session name (or per task) — and points the Browser Use CLI at it. No Chromium, Playwright or Node.js is needed. The process is reaped after `browser.inactivity_timeout`, on exit, and by the orphan sweep if J.A.G.O.D.A crashes. Lightpanda has no graphical renderer, so `capture_screenshot()` is unavailable and the tool description tells the model to work text-first; it also holds one page per session, so the model is told to call `new_tab()` once and `goto_url()` afterwards (tracked upstream in [lightpanda-io/browser#1962](https://github.com/lightpanda-io/browser/issues/1962)).
- **Built-in browser tools** (`/browser use off`). J.A.G.O.D.A drives Lightpanda through `agent-browser --engine lightpanda` over CDP, the same way it drives local Chrome, with **automatic Chrome fallback**: Lightpanda handles the actions it supports (navigate, snapshot, click, type, scroll, back, press, eval) and J.A.G.O.D.A transparently retries on Chrome for anything it doesn't. Screenshots and `browser_vision` are routed straight to Chrome.
...
Instead of a cloud provider, you can attach J.A.G.O.D.A browser tools to your own running Chrome, Brave, Chromium, or Edge instance via the Chrome DevTools Protocol (CDP). This is useful when you want to see what the agent is doing in real-time, interact with pages that require your own cookies/sessions, or avoid cloud browser costs.

:::note
`/browser connect` is an **interactive-CLI slash command** — it is not dispatched by the gateway. If you try to run it inside a WebUI, Telegram, Discord, or other gateway chat, the message will be sent to the agent as plain text and the command will not execute. Start J.A.G.O.D.A from the terminal (`hermes` or `hermes chat`) and issue `/browser connect` there.
:::
...
Then launch the J.A.G.O.D.A CLI and run `/browser connect`.
...
If J.A.G.O.D.A runs inside WSL2 but the Chrome window you want to control runs on the Windows host, `/browser connect` is often not the best path.

Why:

- `/browser connect` expects J.A.G.O.D.A itself to reach a usable CDP endpoint
...
- even when Windows Chrome is debuggable, the cleanest integration is often to let a Windows-side browser MCP server attach to Chrome and let J.A.G.O.D.A talk to that MCP server

For that setup, prefer `chrome-devtools-mcp` through J.A.G.O.D.A MCP support.
...
If you do **not** set any cloud credentials and don't use `/browser connect`, J.A.G.O.D.A can still use the browser tools through a local Chromium install driven by `agent-browser`.
...
# Browser Use mode (default) "lightpanda" makes J.A.G.O.D.A spawn `lightpanda serve`;
...
# Extra Chromium launch flags (comma- or newline-separated). J.A.G.O.D.A auto-injects
...
# J.A.G.O.D.A doesn't add automatically; setting it disables the auto-injection.
...
:::note
If paid features aren't available on your plan, J.A.G.O.D.A automatically falls back — first disabling `keepAlive`, then proxies — so browsing still works on free plans.
:::
