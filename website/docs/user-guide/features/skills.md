...
You can also point J.A.G.O.D.A at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
(Plan mode works the same way but is a built-in command now: `/plan [request]` tells J.A.G.O.D.A to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.hermes/plans/` relative to the active workspace/backend working directory.)
...
description, the standard section order, J.A.G.O.D.A-tool framing, no invented
commands).
...
When a missing value is encountered, J.A.G.O.D.A asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `hermes setup` or `~/.hermes/.env` locally instead.
...
and `examples/`. Unreferenced repository files are not copied. J.A.G.O.D.A scans the
...
policy above), J.A.G.O.D.A can run [NVIDIA SkillEvaluator](https://github.com/NVIDIA/SkillEvaluator)
...
If you maintain skills outside of J.A.G.O.D.A — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell J.A.G.O.D.A to scan those directories too.
...
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the J.A.G.O.D.A process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, J.A.G.O.D.A ignores it without errors. Useful for optional shared directories that may not be present on every machine.
...
Repos can carry their own skills, active only for sessions started inside that project — the same pattern other agent harnesses use for repo-local configuration. When you launch J.A.G.O.D.A inside a git checkout, it looks for skills in:

```text
<project-root>/.hermes/skills/    # J.A.G.O.D.A-native location
...
Skills are procedure documents the agent follows, so J.A.G.O.D.A does **not** auto-load them from arbitrary cloned repos. The first time you run J.A.G.O.D.A in a repo with project skills, the banner shows a notice:
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with J.A.G.O.D.A. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `hermes skills search <query> --source skills-sh`. J.A.G.O.D.A resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
J.A.G.O.D.A currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the J.A.G.O.D.A repository itself and install with built-in trust.
...
This is Vercel's public skills directory. J.A.G.O.D.A can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
J.A.G.O.D.A can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
categorization, no J.A.G.O.D.A-side changes required.
...
- J.A.G.O.D.A source id: `clawhub`

#### 6. LobeHub (`lobehub`)

J.A.G.O.D.A can search and convert agent entries from LobeHub's public catalog into installable J.A.G.O.D.A skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- J.A.G.O.D.A source id: `lobehub`

#### 7. browse.sh (`browse-sh`)

J.A.G.O.D.A integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with J.A.G.O.D.A's browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- J.A.G.O.D.A source id: `browse-sh`
...
Install `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). J.A.G.O.D.A also fetches explicitly referenced files under `references/`, `templates/`, `scripts/`, `assets/`, and `examples/`, then scans and installs the complete bundle.

- J.A.G.O.D.A source id: `url`
- Identifier: the URL itself (no prefix needed)
- Scope: `SKILL.md` plus exact referenced support files in the allowlisted directories. J.A.G.O.D.A does not enumerate or copy unrelated files from the host.
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with J.A.G.O.D.A | Always trusted |
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other J.A.G.O.D.A users add with `hermes skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
J.A.G.O.D.A discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.
...
After pushing that to GitHub, any J.A.G.O.D.A user can subscribe and install:
...
J.A.G.O.D.A ships with a set of bundled skills in `skills/` inside the repo. On install and on every `hermes update`, a sync pass copies those into `~/.hermes/skills/` and records a manifest at `~/.hermes/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, J.A.G.O.D.A recomputes the hash of your local copy and compares it to the origin hash:
