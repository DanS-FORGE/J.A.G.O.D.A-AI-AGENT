...
The curator is triggered by an inactivity check, not a cron daemon. On CLI session start, and on a recurring tick inside the gateway's cron-ticker thread, J.A.G.O.D.A checks whether:
...
Before every real curator pass, J.A.G.O.D.A takes a tar.gz snapshot of `~/.hermes/skills/` at `~/.hermes/skills/.curator_backups/<utc-iso>/skills.tar.gz`. If a pass archives or consolidates something you didn't want touched, you can undo the whole run with one command:
...
The rollback itself is reversible: before replacing the skills tree, J.A.G.O.D.A takes another snapshot tagged `pre-rollback to <target-id>`, so a mistaken rollback can be undone by rolling forward to that one with `--id`.
...
If you manually created a `SKILL.md` or pointed J.A.G.O.D.A at an external skill
...
**wrote** it — J.A.G.O.D.A edits user-authored skills on your behalf constantly. An
