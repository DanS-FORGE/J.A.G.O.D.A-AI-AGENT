...
Type `@` followed by a reference to inject content directly into your message. J.A.G.O.D.A expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- J.A.G.O.D.A env: `$HERMES_HOME/.env`
