...
description: "Customize Jagoda's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Jagoda's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.
...
If you want to change who Jagoda is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

J.A.G.O.D.A now seeds a default `SOUL.md` automatically in:

```text
~/.hermes/SOUL.md
```

More precisely, it uses the current instance's `HERMES_HOME`, so if you run J.A.G.O.D.A with a custom home directory, it will use:
...
- J.A.G.O.D.A creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- J.A.G.O.D.A loads `SOUL.md` only from `HERMES_HOME`
- J.A.G.O.D.A does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, J.A.G.O.D.A falls back to a built-in default identity
...
If J.A.G.O.D.A loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `HERMES_HOME`, the personality belongs to the J.A.G.O.D.A instance itself.

That also makes it easier to teach users:
- "Edit `~/.hermes/SOUL.md` to change Jagoda's default personality."
...
- how Jagoda should handle uncertainty, disagreement, or ambiguity
...
## What J.A.G.O.D.A injects into the prompt
...
J.A.G.O.D.A ships with built-in personalities you can switch to with `/personality`.
...
These are convenient overlays, but your global `SOUL.md` still gives Jagoda its persistent default personality unless the overlay meaningfully changes it.
...
- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Jagoda speaks
- `display.skin` and `/skin` affect how J.A.G.O.D.A looks in the terminal
