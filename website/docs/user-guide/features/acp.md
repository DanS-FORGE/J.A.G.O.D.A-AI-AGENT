...
description: "Use J.A.G.O.D.A inside ACP-compatible editors and collaboration platforms"
---

# ACP Host Integration

J.A.G.O.D.A can run as an ACP server, letting ACP-compatible hosts talk to
J.A.G.O.D.A over stdio. Editors can render:
...
J.A.G.O.D.A. ACP is a good fit when you want J.A.G.O.D.A to keep its existing identity,
provider setup, memory, skills, and tools while another application owns the
conversation transport.

## What J.A.G.O.D.A exposes in ACP mode

J.A.G.O.D.A runs with a curated `hermes-acp` toolset designed for editor workflows. It includes:
...
Install J.A.G.O.D.A normally, then add the ACP extra from the install checkout:
...
Any of the following starts J.A.G.O.D.A in ACP mode:
...
J.A.G.O.D.A logs to stderr so stdout remains reserved for ACP JSON-RPC traffic.
...
- `npm install -g agent-browser @askjo/camofox-browser` into that prefix (no sudo needed — `npm`'s `--prefix` points at the user-writable J.A.G.O.D.A-managed Node)
...
```text
Buzz relay <-- WebSocket --> buzz-acp <-- ACP over stdio --> J.A.G.O.D.A
```

This is a transport integration, not a second J.A.G.O.D.A installation. The
subprocess launched by `buzz-acp` uses the same J.A.G.O.D.A configuration,
...
spawns J.A.G.O.D.A locally as a preset harness. The relay bridge is for joining Buzz
...
- Mint a dedicated Nostr keypair for J.A.G.O.D.A (`buzz-admin generate-key`) and
...
the same operating-system user that owns the intended J.A.G.O.D.A home. Setup,
...
The bridge discovers every Buzz channel where the J.A.G.O.D.A identity is a member
and automatically subscribes when it is added to another channel. Buzz channel
membership therefore remains the access boundary; J.A.G.O.D.A does not need a
separate channel list in its own configuration.

To expose J.A.G.O.D.A ACP activity in the owner's Buzz Desktop, add:
...
where J.A.G.O.D.A is expected to work.
...
2. Select **J.A.G.O.D.A** from the built-in agent list.
3. Connect and start chatting.

If you want to define J.A.G.O.D.A manually, add it through VS Code settings under `acp.agents`:
...
Configure J.A.G.O.D.A as a custom agent server in Zed settings:
...
3. Start a new J.A.G.O.D.A external-agent thread.

Prerequisites:

- Configure J.A.G.O.D.A provider credentials first with `hermes model`, or set them in `~/.hermes/.env` / `~/.hermes/config.yaml`.
...
[Buzz](https://github.com/block/buzz) ships J.A.G.O.D.A as a preset runtime.
With J.A.G.O.D.A installed the normal way, Buzz discovers it automatically —
open **Settings → Runtimes** and J.A.G.O.D.A appears under your runtimes.
...
Buzz Desktop (v0.5.1+) renders J.A.G.O.D.A's full model menu in the agent's runtime
settings. The list comes from J.A.G.O.D.A itself over ACP: it shows every model
from providers you have authenticated in J.A.G.O.D.A (the same inventory behind
`hermes model` and the `/model` command), so a model missing from the menu
means its provider has no credentials configured on the J.A.G.O.D.A side.
...
change your J.A.G.O.D.A-wide default — use `hermes model` for that.

#### Keep Buzz agents owner-only

Buzz creates every agent with **Who can talk to this agent** set to `Owner only`.
Leave it there when the runtime is J.A.G.O.D.A.

Two behaviors combine on this path. The `hermes-acp` toolset includes `terminal`
and `execute_code`, and Buzz's ACP bridge answers J.A.G.O.D.A's permission requests
itself with `allow_once` rather than surfacing them. A J.A.G.O.D.A agent in Buzz
...
- `approvals.mode: manual` does make J.A.G.O.D.A raise the permission request, but
...
ACP mode uses the same J.A.G.O.D.A configuration as the CLI:
...
Provider resolution uses J.A.G.O.D.A's normal runtime resolver, so ACP inherits the currently configured provider and credentials. J.A.G.O.D.A also advertises a terminal auth method (`--setup`) for first-run ACP clients; this opens J.A.G.O.D.A's interactive model/provider setup.

## Host integration

These variables are set by an **ACP host process** (an editor or another agent
harness) on the J.A.G.O.D.A subprocess it spawns. They are not user configuration —
...
J.A.G.O.D.A normally starts every MCP server configured in `config.yaml` before it
...
The underlying `AIAgent` still uses J.A.G.O.D.A's normal persistence/logging paths, but ACP `list/load/resume/fork` are scoped to the currently running ACP server process.

## Working directory behavior

ACP sessions bind the editor's cwd to the J.A.G.O.D.A task ID so file and terminal tools run relative to the editor workspace, not the server process cwd.
...
| `allow_always` | Allow always | All future sessions | Yes (written to the J.A.G.O.D.A permanent allowlist) |
| `deny` | Deny | This one tool call | No |

`allow_session` is the right default for an editor workflow where you trust an agent for the duration of a task but don't want to grant a long-lived allowlist entry. The safety trade-off is straightforward: the broader the scope, the less the editor will interrupt you, and the more damage a misbehaving agent (or prompt injection) can do before you notice. Start with `allow_once` for unfamiliar commands; promote to `allow_session` once you've seen the agent run the same pattern correctly a few times; reserve `allow_always` for truly idempotent commands you trust forever (e.g. `git status`).

The ACP bridge maps these options onto J.A.G.O.D.A's internal approval semantics — `allow_always` writes a permanent allowlist entry the same way the CLI does, while `allow_session` only affects the in-process approval cache for the current ACP session.
...
- J.A.G.O.D.A is installed and on your PATH.
...
ACP mode uses J.A.G.O.D.A's existing provider setup. Configure credentials with:
