...
description: "Build themes and plugins for the J.A.G.O.D.A web dashboard — palettes, typography, layouts, custom tabs, shell slots, page-scoped slots, and backend API routes"
---

# Extending the Dashboard

The J.A.G.O.D.A web dashboard (`hermes dashboard`) is built to be reskinned and extended without forking the codebase. Three layers are exposed:
...
|-------|---------|------------|--------|
| **J.A.G.O.D.A Teal** (`default`) | Dark teal + cream | System stack, 15px | 0.5rem radius, comfortable |
| **J.A.G.O.D.A Teal (Large)** (`default-large`) | Same as default | System stack, 18px, line-height 1.65 | 0.5rem radius, spacious |
...
Themes that reference Google Fonts (all except J.A.G.O.D.A Teal) load the stylesheet on demand — the first time you switch to them a `<link>` tag is injected into `<head>`.
...
// J.A.G.O.D.A API client + raw fetcher
...
#### Calling built-in J.A.G.O.D.A endpoints
...
| `header-left` | Before the J.A.G.O.D.A brand in the top bar. |
...
#### Accessing J.A.G.O.D.A internals
...
Open the dashboard, pick **Strike Freedom** from the theme switcher. The cockpit sidebar appears, the crest shows in the header, the tagline replaces the footer. Switch back to **J.A.G.O.D.A Teal** and the plugin remains installed but invisible (the `sidebar` slot only renders under the `cockpit` layout variant).
