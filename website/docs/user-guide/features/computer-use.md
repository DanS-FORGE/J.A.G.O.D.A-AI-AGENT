...
J.A.G.O.D.A can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended J.A.G.O.D.A integration. It
...
**Fresh installs already have the driver.** The J.A.G.O.D.A installer
...
Already have cua-driver? J.A.G.O.D.A reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `hermes update`, and the first
`computer_use` call of a session, J.A.G.O.D.A checks the local version and
...
under your control, so J.A.G.O.D.A reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. J.A.G.O.D.A autodetection is a
planned cua-driver follow-up, so currently point J.A.G.O.D.A at that directory or
...
interface. The built-in toolset provides J.A.G.O.D.A actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `hermes computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the J.A.G.O.D.A host identity. |
...
J.A.G.O.D.A maps its existing approval UX onto cua-driver's immutable runtime
modes. Permission mode and capability manifest approval are launch settings.
They cannot change after the runtime starts:

| J.A.G.O.D.A session | cua-driver mode | Human intervention |
|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal J.A.G.O.D.A approvals; Cua stops at its protected boundary |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit J.A.G.O.D.A risk acceptance; no runtime Cua prompts |
...
for the format). J.A.G.O.D.A launches a private runtime with
...
identity instead of resetting with every J.A.G.O.D.A build), and J.A.G.O.D.A verifies
...
resetting or closing the J.A.G.O.D.A session, cancellation cleanup, or process exit
closes that transport session. J.A.G.O.D.A also stops private runtimes that it
launched for bounded or unrestricted access. One J.A.G.O.D.A
conversation cannot change another runtime's mode or grants. Bounded and
unrestricted modes use a private
embedded service under the J.A.G.O.D.A host identity.
...
J.A.G.O.D.A run declares a public cua-driver **session name** (something like
...
without it. J.A.G.O.D.A disables it automatically where it is a known failure
...
J.A.G.O.D.A keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the J.A.G.O.D.A-side `computer_use` workflow and action vocabulary. For
...
The command installs the pack under `~/.cua-driver/skills/cua-driver`. J.A.G.O.D.A
autodetection is a planned cua-driver follow-up, so currently point J.A.G.O.D.A at
...
These are **platform deep dives, not duplicates of the J.A.G.O.D.A skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **J.A.G.O.D.A
...
image capture also saves a bounded, shareable copy under J.A.G.O.D.A's image cache
...
J.A.G.O.D.A applies multi-layer guardrails:
...
Screenshots are expensive. J.A.G.O.D.A applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  J.A.G.O.D.A agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the J.A.G.O.D.A agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. J.A.G.O.D.A warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive J.A.G.O.D.A from inside the RDP / console
...
upstream. **J.A.G.O.D.A disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) J.A.G.O.D.A sets
...
unreleased fix — point J.A.G.O.D.A at a binary you built from source instead
of the published release. J.A.G.O.D.A resolves the driver with
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for J.A.G.O.D.A testing (see notes).
...
### Option B — point J.A.G.O.D.A straight at the built binary (fastest loop)
...
### Confirm J.A.G.O.D.A is using your build
...
- **J.A.G.O.D.A spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  J.A.G.O.D.A YOLO, J.A.G.O.D.A instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no J.A.G.O.D.A
  change needed either way.
- **Local builds skip the version check.** J.A.G.O.D.A warns when the
...
- **J.A.G.O.D.A-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  J.A.G.O.D.A `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. J.A.G.O.D.A autodetection is a
  planned follow-up; currently point J.A.G.O.D.A at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (J.A.G.O.D.A among them)
