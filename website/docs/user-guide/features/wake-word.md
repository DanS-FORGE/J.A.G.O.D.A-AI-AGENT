...
sidebar_position: 11
title: "Wake Word"
description: "Hands-free 'Hey J.A.G.O.D.A' wake word — start a voice session by speaking, the 'Hey Siri' way"
---

# Wake Word ("Hey J.A.G.O.D.A")

The wake word turns J.A.G.O.D.A into a hands-free assistant across the CLI, TUI, and
desktop app: with one setting on, J.A.G.O.D.A listens in the background for a spoken
trigger phrase. Say it, and J.A.G.O.D.A starts a fresh session, opens the microphone,
...
When the desktop app connects to a **remote** J.A.G.O.D.A backend (for example a
...
J.A.G.O.D.A supports **client capture** for that case:
...
By default the phrase is **"hey hermes"** — a model for it ships with J.A.G.O.D.A, so
...
default) to let J.A.G.O.D.A choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which J.A.G.O.D.A installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three J.A.G.O.D.A surfaces, and `surface` picks which one
...
including when J.A.G.O.D.A surfaces run in separate processes. Ownership is sticky:
the first eligible claimant keeps the listener until it stops, disconnects, or
its process exits. J.A.G.O.D.A does not silently fail over to another open surface.
...
"Hey J.A.G.O.D.A" works out of the box — the bundled openWakeWord model
...
Create a "Hey J.A.G.O.D.A" keyword in the [Picovoice Console](https://console.picovoice.ai/),
...
but the phrase never fires. J.A.G.O.D.A detects this (`/wake status` shows
"mic delivers only silence"; the desktop ear tooltip carries the same hint).
Fix: System Settings → Privacy & Security → Microphone → enable the J.A.G.O.D.A
backend (it may appear as your terminal, `python`, or J.A.G.O.D.A), then toggle the
