...
**Tool Search** is J.A.G.O.D.A's opt-in progressive-disclosure layer for that
...
:::info Built-in J.A.G.O.D.A tools never defer
The tools that make up J.A.G.O.D.A's core capability set (`terminal`,
...
When the model invokes `tool_call`, J.A.G.O.D.A **unwraps the bridge** and
...
- **No JS sandbox.** J.A.G.O.D.A uses the simpler "structured tools" mode
