---
sidebar_position: 15
title: "J.A.G.O.D.A Web Dashboard"
...
# J.A.G.O.D.A Web Dashboard

The web dashboard is a browser-based UI for managing your J.A.G.O.D.A installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.
...
Create and manage [profiles](../profiles.md) — isolated J.A.G.O.D.A instances with their own config, skills, and sessions.
...
Connect J.A.G.O.D.A to any messaging platform from the browser — full parity with
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and J.A.G.O.D.A versions, CPU core count + utilization, memory, disk usage of the J.A.G.O.D.A home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The J.A.G.O.D.A version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `hermes update` in the background. On Docker/Nix installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
...
When the dashboard is bound to a public or non-loopback address — anything other than `127.0.0.1` / `localhost` — J.A.G.O.D.A engages an auth gate. Every request must carry a verified session cookie or it's bounced to the login page. Three providers ship in the box:
...
From a logged-in J.A.G.O.D.A install to a Nous-gated dashboard in three steps.
...
first; without one, J.A.G.O.D.A fails closed at startup. This prevents the local SPA
...
J.A.G.O.D.A always preserves loopback trust and rejects `*`, `0.0.0.0/0`, and
...
J.A.G.O.D.A Desktop can drive a J.A.G.O.D.A backend running on another machine (a VPS, a home server, a Mini behind Tailscale). In the app this lives under **Settings → Gateways → Remote gateway**, which asks for a **Remote URL** and a way to **Sign in**. (For the desktop app itself — install, settings, chat — see the [J.A.G.O.D.A Desktop](/user-guide/desktop) page.)
...
| Theme | Character |
|-------|-----------|
| **J.A.G.O.D.A Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **J.A.G.O.D.A Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
