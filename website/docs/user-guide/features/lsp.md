...
J.A.G.O.D.A runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. J.A.G.O.D.A
...
1. J.A.G.O.D.A captures a baseline of current diagnostics for the file.
...
…). J.A.G.O.D.A auto-detects the binary on PATH or in
`<HERMES_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point J.A.G.O.D.A at the extracted bundle — the directory that contains
...
`node_modules` tree — J.A.G.O.D.A installs both packages together when you
...
When `install_strategy: auto`, J.A.G.O.D.A installs binaries into
...
shared location — the staging dir is fully J.A.G.O.D.A-owned and is
