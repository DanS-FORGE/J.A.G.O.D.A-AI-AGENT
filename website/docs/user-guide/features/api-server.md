...
# Optional: only if a browser must call J.A.G.O.D.A directly
...
**Streaming** (`"stream": true`): Returns Server-Sent Events (SSE) with token-by-token response chunks. For **Chat Completions**, the stream uses standard `chat.completion.chunk` events plus J.A.G.O.D.A's custom `hermes.tool.progress` event for tool-start UX. For **Responses**, the stream uses OpenAI Responses event types such as `response.created`, `response.output_text.delta`, `response.output_item.added`, `response.output_item.done`, and `response.completed`.

**Tool progress in streams**:
- **Chat Completions**: J.A.G.O.D.A emits `event: hermes.tool.progress` for tool-start visibility without polluting persisted assistant text.
- **Responses**: J.A.G.O.D.A emits spec-native `function_call` and `function_call_output` output items during the SSE stream, so clients can render structured tool UI in real time.
...
Tool calls in the `output` array were already executed server-side by the J.A.G.O.D.A agent — they are replayed with `"status": "completed"` for structured tool UI, never as pending calls for the client to execute.
...
enumerate every authenticated provider/model combination J.A.G.O.D.A can route to,
and it does not do pricing or capability enrichment.

### GET /api/model/options

J.A.G.O.D.A-aware clients can request the same curated provider/model inventory used
...
Normal opens are intentionally conservative for custom providers: J.A.G.O.D.A probes
...
authenticated UI needs the richer J.A.G.O.D.A-specific picker metadata.
...
Use this endpoint when integrating dashboards, browser UIs, or control planes so they can discover whether the running J.A.G.O.D.A version supports runs, streaming, cancellation, and session continuity without depending on private Python internals.

## Browser-extension control

J.A.G.O.D.A can route browser tools through an authenticated extension that controls
the browser session associated with the current J.A.G.O.D.A session. The feature is
...
register only for an existing server session. J.A.G.O.D.A derives the controller
...
J.A.G.O.D.A preserves the existing browser backend. Once the gateway binds a
...
J.A.G.O.D.A never retries the same action through another backend.
...
2. J.A.G.O.D.A returns a single-use ticket with a 30-second TTL and the filtered,
...
J.A.G.O.D.A sends `browser.controller.command` frames containing `command_id`,
...
Authenticated clients can override J.A.G.O.D.A's default model selection per request
by sending:

- `model` — the target model id for this turn
- `provider` — the J.A.G.O.D.A provider slug to resolve credentials/runtime for this turn
...
alias, J.A.G.O.D.A rejects the request with `400` instead of silently remixing route
...
Requests that include an explicit `provider` — and the J.A.G.O.D.A-native
...
Runs accept a simple `input` string and optional `session_id`, `instructions`, `conversation_history`, or `previous_response_id`. When `session_id` is provided, J.A.G.O.D.A surfaces it in the run status so external UIs can correlate runs with their own conversation IDs.

For safely retryable creation, send an `Idempotency-Key` header (1–255 visible ASCII characters). J.A.G.O.D.A durably reserves the key before starting work. An identical retry returns the original `run_id` with HTTP 202 and `Idempotency-Replayed: true`, including after a gateway restart and after the run has completed, failed, or been cancelled. Reusing the same key with a different JSON payload returns HTTP 409 with code `idempotency_key_conflict`. Keys are isolated by authenticated API profile/credential and retained for 24 hours after their last status update; clients should use unique, unguessable keys and must not reuse them for unrelated operations. Requests without the header retain the legacy behavior and always create a new run.

When `session_id` identifies an existing J.A.G.O.D.A session and no explicit
...
Interrupt a running agent turn. The endpoint returns immediately with `{"status": "stopping"}` while J.A.G.O.D.A asks the active agent to stop at the next safe interruption point.
...
External UIs can manage J.A.G.O.D.A sessions over REST without standing up the dashboard. All endpoints are gated by `API_SERVER_KEY` and live under `/api/sessions/*`.
...
Multi-user frontends like Open WebUI need a stable per-channel identifier for long-term memory (Honcho, etc.) that is **independent** of the transcript-scoped `X-Hermes-Session-Id` (which rotates on `/new`). Pass `X-Hermes-Session-Key` on `/v1/chat/completions`, `/v1/responses`, or `/v1/runs` and J.A.G.O.D.A threads it through to `AIAgent(gateway_session_key=...)`, where the Honcho memory provider uses it to derive a stable scope.
...
Rules: max 256 chars, control characters (`\r`, `\n`, `\x00`) are rejected, and the value is echoed back on responses (JSON + SSE). `/v1/capabilities` advertises support via `"session_key_header": "X-Hermes-Session-Key"`. Without the key, Honcho's `per-session` strategy produces a different scope per `session_id` — exactly the behavior J.A.G.O.D.A had before.
...
Configure the key via `API_SERVER_KEY` env var. If you need a browser to call J.A.G.O.D.A directly, also set `API_SERVER_CORS_ORIGINS` to an explicit allowlist.
...
To give multiple users their own isolated J.A.G.O.D.A instance (separate config, memory, skills), use [profiles](/user-guide/profiles):
...
In Open WebUI, add each as a separate connection. The model dropdown shows `alice` and `bob` as distinct models, each backed by a fully isolated J.A.G.O.D.A instance. See the [Open WebUI guide](/user-guide/messaging/open-webui#multi-user-setup-with-profiles) for details.
