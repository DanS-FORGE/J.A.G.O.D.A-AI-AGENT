...
J.A.G.O.D.A generates images from text prompts via FAL.ai. Eleven models are supported out of the box, each with different speed, quality, and cost tradeoffs. The active model is user-configurable via `hermes tools` and persists in `config.yaml`.
...
`max_parallel_requests` defaults to `4`. J.A.G.O.D.A clamps it to at least one and
...
new models appear in the picker as soon as OpenRouter serves them; no J.A.G.O.D.A
...
as a tool the chat model may call, and J.A.G.O.D.A cannot force the call — the
...
- **Temporary URLs** — backends return hosted URLs that expire after hours/days; J.A.G.O.D.A materializes them to the local cache so delivery still works after expiry
