...
files. It is **never persisted** to any `.env` by J.A.G.O.D.A.
...
If you try to change a managed value, J.A.G.O.D.A refuses and names the source:
...
Changes take effect on the next J.A.G.O.D.A start (a malformed managed file is logged
...
- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs J.A.G.O.D.A as `root`), managed scope is advisory.
