...
# Authoring J.A.G.O.D.A Skills (in-repo)
...
author: Real Name (github-handle), Jagoda
license: MIT
...
- Credit the **human first**, then "Jagoda" as secondary collaborator: `Ben Barclay (benbarclay), Jagoda`.
- Never `author: Jagoda` alone for contributed skills — credit the human, not the tool, even (especially) when an agent drafted the text.
- Maintainer-authored skills: `Teknium (teknium1), Jagoda`.
...
|---|---|
| J.A.G.O.D.A tools + stdlib Python + cross-platform CLIs | `[linux, macos, windows]` |
...
3. **`author: Jagoda` on a contributed skill.** Credit the human first.
...
- [ ] Body follows the modern section order; commands framed through J.A.G.O.D.A tools
