...
J.A.G.O.D.A can automatically snapshot your project before **destructive operations** and restore it with a single command. Checkpoints are **opt-in** as of v2 — most users never use `/rollback`, and the shadow-store storage is non-trivial over time, so the default is off.
...
- J.A.G.O.D.A detects when tools are about to **modify files** in your working tree.
...
J.A.G.O.D.A responds with a formatted list showing change statistics:
...
`/rollback <N>` restores only the files J.A.G.O.D.A itself changed. Every successful
...
J.A.G.O.D.A last wrote (you edited it afterwards, or J.A.G.O.D.A never touched it) is
...
If the ledger is empty (a store created before this feature, or J.A.G.O.D.A hasn't
...
- **Directory scope** — J.A.G.O.D.A skips overly broad directories (root `/`, home `$HOME`).
...
- **Combine with Git worktrees** for maximum safety — keep each J.A.G.O.D.A session in its own worktree/branch, with checkpoints as an extra layer.
