...
sidebar_position: 19
title: "Raft"
description: "Connect J.A.G.O.D.A to Raft as an external agent via wake-channel bridge"
---

# Raft Setup

J.A.G.O.D.A connects to [Raft](https://raft.build) as an external agent through a local wake-channel bridge. The adapter starts a loopback HTTP endpoint that receives content-free wake hints from the bridge, then injects them into the J.A.G.O.D.A gateway session pipeline. The agent reads and sends messages through the Raft CLI — the adapter never touches message bodies or delivery cursors.
...
- **The J.A.G.O.D.A adapter** owns: a localhost wake endpoint and injecting a short notice into the agent's context.
...
- **aiohttp** — Python package (included in J.A.G.O.D.A `[all]` extras)

In Raft, open the Agents menu, create an External Agent, and follow the setup card to install the Raft CLI and log in the agent profile. Once the agent is created, Raft shows a J.A.G.O.D.A setup guide with the environment variables and configuration needed to start the gateway.
...
```
Raft Server → Bridge (wake-hints SSE) → POST /wake → J.A.G.O.D.A Adapter → Agent context
...
3. The adapter validates the bridge token, verifies the payload is content-free, and injects a wake notice into the J.A.G.O.D.A session.
