...
Connect J.A.G.O.D.A to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
J.A.G.O.D.A supports two WeCom integration modes:
...
3. J.A.G.O.D.A decrypts the message, queues it for the agent
...
console. A mismatch between the token configured in J.A.G.O.D.A and the token the
