...
sidebar_position: 4
title: "Slack"
description: "Set up J.A.G.O.D.A as a Slack bot using Socket Mode"
---

# Slack Setup

Connect J.A.G.O.D.A to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your J.A.G.O.D.A instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. J.A.G.O.D.A uses the modern
...
The fastest path is to paste a manifest J.A.G.O.D.A generates for you. It
...
### Option A: From a J.A.G.O.D.A-generated manifest (recommended)
...
it will only work in DMs. Without `files:read`, J.A.G.O.D.A can chat but **cannot reliably read user-uploaded attachments**.
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a J.A.G.O.D.A configuration issue.
:::
...
J.A.G.O.D.A uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
can spend the entire turn in hidden reasoning and never produce visible assistant text; J.A.G.O.D.A now
...
Every J.A.G.O.D.A command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
J.A.G.O.D.A command with its description.

Under the hood: J.A.G.O.D.A ships with a generated Slack app manifest (see
...
New Slack apps use Slack's **Agent** messaging experience. Existing J.A.G.O.D.A
...
to `message.im`, `app_home_opened`, and `app_context_changed`, so J.A.G.O.D.A can
identify a Messages-tab DM and receive the user's active Slack context with a
turn. J.A.G.O.D.A only supplies that context as a label; it does not read the viewed
channel's history.

### Refreshing slash commands after updates

When J.A.G.O.D.A adds new commands (e.g. after `hermes update`), regenerate
...
`/hermes bg run the tests` — J.A.G.O.D.A routes it the same way as `/bg
...
Slack never delivers them to J.A.G.O.D.A.

As a workaround, J.A.G.O.D.A recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
J.A.G.O.D.A treats it identically to the slash form and replies in the same
thread.
...
J.A.G.O.D.A falls back to a text prompt, the prompt instructs you to reply
...
fails, J.A.G.O.D.A retries via a second ephemeral API path — a slash reply is never
...
Understanding how J.A.G.O.D.A behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Hermes Agent what time is it?`). In channels, J.A.G.O.D.A replies in a thread attached to that message. |
| **Threads** | If you @mention J.A.G.O.D.A inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
platforms:
  slack:
    ...
    extra:
      ...
      # If Slack rejects the native stream, J.A.G.O.D.A keeps one editable text
      ...
      # that message itself @mentions J.A.G.O.D.A, and "all" accepts every
      # other bot. J.A.G.O.D.A always ignores its own bot user to prevent
      # self-echoes.
      allow_bots: "none"
...
| `platforms.slack.extra.allow_bots` | `"none"` | Controls messages from other Slack bots: `"none"` ignores them, `"mentions"` accepts a bot message only when **that message itself** @mentions J.A.G.O.D.A, and `"all"` accepts all of them. Use `"mentions"` for the safest bot-to-bot collaboration mode. See [Accepting messages from other bots](#accepting-messages-from-other-bots-allow_bots). |
...
name in the thread. By default J.A.G.O.D.A sets it to `is thinking...`; customize it
...
(`platforms.google_chat.typing_status_text`, default `"J.A.G.O.D.A is thinking…"`) —
...
`streaming.enabled` is on (transport `auto` or `draft`), J.A.G.O.D.A uses native
streaming automatically wherever it's available:

- The stream starts on the first frame and appends only deltas (the API is
  append-only). The streamed message **is** the final message — J.A.G.O.D.A seals
...
- If your Slack app doesn't have the AI features enabled (or lacks the
  `assistant:write` scope), the first failure is cached and J.A.G.O.D.A falls back
...
- If the native stream can't start or update, J.A.G.O.D.A falls back to a single
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to J.A.G.O.D.A in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before J.A.G.O.D.A will respond.
...
By default J.A.G.O.D.A ignores every message authored by another Slack bot or app (including Workflow Builder posts). For multi-agent workspaces — several J.A.G.O.D.A instances or peer bots collaborating in one channel — opt in with `allow_bots`:
...
- J.A.G.O.D.A always ignores its own messages, in every mode, to prevent self-echo loops.

`mentions` is the recommended mode for bot-to-bot collaboration: each agent must explicitly summon the other per turn. Avoid `all` unless every peer bot's own reply policy is loop-safe — two bots that answer everything will answer each other forever. Detection covers labeled bot messages (`bot_id`, `subtype: bot_message`), app-originated events, and unlabeled bot *users* (probed via `users.info`), so peer J.A.G.O.D.A agents are filtered consistently across workspaces.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where J.A.G.O.D.A will deliver scheduled messages,
...
Delivery works even when the cron process isn't co-located with the gateway — J.A.G.O.D.A falls back to a standalone Web API sender using `SLACK_BOT_TOKEN`. `MEDIA:` attachments in the cron output are uploaded as native Slack file shares to the same target.
...
J.A.G.O.D.A can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, J.A.G.O.D.A also loads tokens from an **OAuth token file** at:
...
- When a message arrives, J.A.G.O.D.A uses the correct workspace-specific client to respond.
...
J.A.G.O.D.A supports voice on Slack:
...
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. J.A.G.O.D.A now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
- Audit who has access to your J.A.G.O.D.A config directory
