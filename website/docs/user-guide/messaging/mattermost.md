...
sidebar_position: 8
title: "Mattermost"
description: "Set up J.A.G.O.D.A as a Mattermost bot"
---

# Mattermost Setup

J.A.G.O.D.A integrates with Mattermost as a bot, letting you chat with your AI assistant through direct messages or team channels. Mattermost is a self-hosted, open-source Slack alternative — you run it on your own infrastructure, keeping full control of your data. The bot connects via Mattermost's REST API (v4) and WebSocket for real-time events, processes messages through the J.A.G.O.D.A pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, file attachments, images, and slash commands.

No external Mattermost library is required — the adapter uses `aiohttp`, which is already a J.A.G.O.D.A dependency.

Before setup, here's the part most people want to know: how J.A.G.O.D.A behaves once it's in your Mattermost instance.
...
| **Public/private channels** | J.A.G.O.D.A responds when you `@mention` it. Without a mention, J.A.G.O.D.A ignores the message. |
| **Threads** | If `MATTERMOST_REPLY_MODE=thread`, J.A.G.O.D.A replies in a thread under your message. Thread context stays isolated from the parent channel. |
| **Shared channels with multiple users** | By default, J.A.G.O.D.A isolates session history per user inside the channel. Two people talking in the same channel do not share one transcript unless you explicitly disable that. |

:::tip
If you want J.A.G.O.D.A to reply as threaded conversations (nested under your original message), set `MATTERMOST_REPLY_MODE=thread`. The default is `off`, which sends flat messages in the channel.
:::
...
:::tip
You can also use a **personal access token** instead of a bot account. Go to **Profile** → **Security** → **Personal Access Tokens** → **Create Token**. This is useful if you want J.A.G.O.D.A to post as your own user rather than a separate bot user.
:::
...
J.A.G.O.D.A uses your Mattermost User ID to control who can interact with the bot. To find it:
...
The `MATTERMOST_REPLY_MODE` setting controls how J.A.G.O.D.A posts responses:

| Mode | Behavior |
|------|----------|
| `off` (default) | J.A.G.O.D.A posts flat messages in the channel, like a normal user. |
| `thread` | J.A.G.O.D.A replies in a thread under your original message. Keeps channels clean when there's lots of back-and-forth. |
...
- **No extra dependencies**: The adapter uses `aiohttp` for HTTP and WebSocket, which is already included with J.A.G.O.D.A.
