...
sidebar_position: 12
title: "Google Chat"
description: "Set up J.A.G.O.D.A as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect J.A.G.O.D.A to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your J.A.G.O.D.A
...
download the file. Save it somewhere only J.A.G.O.D.A can read (e.g.,
...
Also grant `Pub/Sub Viewer` on the same subscription — J.A.G.O.D.A calls
...
`ADDED_TO_SPACE` event that J.A.G.O.D.A uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure J.A.G.O.D.A
...
Send "hola" in the test DM. The bot posts a "J.A.G.O.D.A is thinking…" marker, then
...
platforms:
  google_chat:
    # Custom working-state marker text (default: "J.A.G.O.D.A is thinking…").
...
Thread support: when a user replies inside a thread, J.A.G.O.D.A detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate J.A.G.O.D.A session.
...
3. Download the JSON. Move it onto the host that runs J.A.G.O.D.A.
4. Register the client with J.A.G.O.D.A (run under the profile you want it scoped to):
...
That writes the client secret into the active profile's J.A.G.O.D.A home (e.g.
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, J.A.G.O.D.A isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
- **Attachment download protection**: J.A.G.O.D.A will only attach the SA bearer
