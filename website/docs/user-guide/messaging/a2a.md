...
[A2A](https://a2a-protocol.org) is the open Agent2Agent protocol (v1.0, stewarded by the Linux Foundation) for communication between independent AI agents. The J.A.G.O.D.A A2A plugin works in **both directions**: your agent can call other A2A agents as tools, and other agents can send tasks to your J.A.G.O.D.A over HTTP.

It interoperates with any A2A-compliant peer — another J.A.G.O.D.A, LangChain, CrewAI, Google ADK agents, or anything built on the official `a2a-sdk`.

## When to use A2A

- **J.A.G.O.D.A ↔ J.A.G.O.D.A across machines** — let your desktop agent hand tasks to a J.A.G.O.D.A on a server, or vice versa, each with its own memory, tools, and credentials.
...
- **Being a callable service** — expose your J.A.G.O.D.A so other frameworks' agents can send it tasks.
...
With the platform enabled, J.A.G.O.D.A serves:
