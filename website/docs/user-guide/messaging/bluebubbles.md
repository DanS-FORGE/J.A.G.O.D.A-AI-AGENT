...
Connect J.A.G.O.D.A to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between J.A.G.O.D.A and the BlueBubbles server
...
By default, J.A.G.O.D.A responds to every authorized BlueBubbles/iMessage DM or group message. To make group chats opt-in, enable mention gating:
...
With `require_mention: true`, DMs still work normally, but group-chat messages are ignored unless they match a mention pattern. If you do not configure custom patterns, J.A.G.O.D.A uses conservative defaults for `Hermes` and `@Hermes agent` variants.
...
When someone messages your iMessage, J.A.G.O.D.A automatically sends them a pairing code. Approve it with:
```bash
...
J.A.G.O.D.A will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → J.A.G.O.D.A
J.A.G.O.D.A → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** J.A.G.O.D.A sends messages via the BlueBubbles REST API.
...
### Chat Addressing
You can address chats by email or phone number — J.A.G.O.D.A resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
