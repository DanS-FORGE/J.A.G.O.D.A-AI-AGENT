...
sidebar_position: 9
title: "Matrix"
description: "Set up J.A.G.O.D.A as a Matrix bot"
---

# Matrix Setup

J.A.G.O.D.A integrates with Matrix, the open, federated messaging protocol. Matrix lets you run your own homeserver or use a public one like matrix.org — either way, you keep control of your communications. The bot connects via the `mautrix` Python SDK, processes messages through the J.A.G.O.D.A pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, file attachments, images, audio, video, and optional end-to-end encryption (E2EE).

J.A.G.O.D.A works with any Matrix homeserver — Synapse, Conduit, Dendrite, or matrix.org.

Before setup, here's the part most people want to know: how J.A.G.O.D.A behaves once it's connected.

## How J.A.G.O.D.A Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | J.A.G.O.D.A responds to every message. No `@mention` needed. Each DM has its own session. Set `MATRIX_DM_MENTION_THREADS=true` to start a thread when the bot is `@mentioned` in a DM. |
| **Rooms** | By default, J.A.G.O.D.A requires an `@mention` to respond. Set `MATRIX_REQUIRE_MENTION=false` or add room IDs to `MATRIX_FREE_RESPONSE_ROOMS` for free-response rooms. Room invites are auto-accepted. |
| **Threads** | J.A.G.O.D.A supports Matrix threads (MSC3440). If you reply in a thread, J.A.G.O.D.A keeps the thread context isolated from the main room timeline. Threads where the bot has already participated do not require a mention. |
| **Auto-threading** | By default, J.A.G.O.D.A auto-creates a thread for each message it responds to in a room. This keeps conversations isolated. Set `MATRIX_AUTO_THREAD=false` to disable. Set `MATRIX_DM_AUTO_THREAD=true` (default false) to also auto-create threads for DM messages — this is distinct from `MATRIX_DM_MENTION_THREADS`, which only starts a thread when the bot is `@mentioned` in a DM. |
| **Commands** | J.A.G.O.D.A accepts normal `/commands` when your Matrix client sends them. If your client reserves `/` for local commands, use `!commands` instead; J.A.G.O.D.A normalizes known `!command` aliases to `/command`. |
...
| **Shared rooms with multiple users** | By default, J.A.G.O.D.A isolates session history per user inside the room. Two people talking in the same room do not share one transcript unless you explicitly disable that. |
...
J.A.G.O.D.A sends structured Matrix user mentions for explicit Matrix IDs such as `@alice:example.org`. Room-wide `@room` notifications are disabled by default; set `MATRIX_ALLOW_ROOM_MENTIONS=true` only in rooms where the bot is allowed to notify everyone.
:::
...
J.A.G.O.D.A now includes the current Matrix room name, room ID, topic, message ID,
...
You can also run J.A.G.O.D.A as your own user. This means the bot posts as you — useful for personal assistants.

## Step 2: Get an Access Token

J.A.G.O.D.A needs an access token to authenticate with the homeserver. You have two options:
...
Instead of providing an access token, you can give J.A.G.O.D.A the bot's user ID and password. J.A.G.O.D.A will log in automatically on startup. This is simpler but means the password is stored in your `.env` file.
...
J.A.G.O.D.A uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
Bridge and appservice deployments need extra loop protection. J.A.G.O.D.A always
...
J.A.G.O.D.A supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.
...
When E2EE is enabled, J.A.G.O.D.A:
...
In Matrix conversations, J.A.G.O.D.A exposes Matrix-specific tools to the agent:
...
Set `MATRIX_APPROVAL_REQUIRE_SENDER=false` if you intentionally want any authorized Matrix user in the room to operate an approval/model picker prompt. The default is requester-bound when J.A.G.O.D.A knows who requested the action.

### Media Limits

J.A.G.O.D.A uploads and downloads Matrix images, files, audio, and video through Matrix media APIs. Multiple generated images are sent as one ordered logical batch, preserving captions and thread context across the batch.
...
Inbound media must use Matrix `mxc://` content URIs. J.A.G.O.D.A rejects arbitrary
...
On each startup, if `MATRIX_RECOVERY_KEY` is set, J.A.G.O.D.A imports cross-signing keys from the homeserver's secure secret storage and signs the current device. This is idempotent and safe to leave enabled permanently.

If J.A.G.O.D.A bootstraps a new Matrix recovery key, it never logs the raw key. Set
...
J.A.G.O.D.A detects this condition on startup and refuses to enable E2EE, logging: `device XXXX has stale one-time keys on the server signed with a previous identity key`.
...
2. Delete the local crypto store and restart J.A.G.O.D.A:
   ```bash
...
J.A.G.O.D.A supports the same gateway commands in Matrix that it supports on other
...
J.A.G.O.D.A only normalizes `!command` when the command is known to the gateway, a
...
If you previously used J.A.G.O.D.A with `MATRIX_ENCRYPTION=true` and are upgrading to
...
a working E2EE setup with a previous version of J.A.G.O.D.A and are upgrading.
...
| Setting | Description |
|---------|-------------|
| `GATEWAY_PROXY_URL` | URL of the remote J.A.G.O.D.A API server (e.g., `http://192.168.1.100:8642`) |
...
**Fix**: J.A.G.O.D.A uses an explicit sync loop that calls `client.handle_sync()` on
both the initial sync and every incremental sync response. This matches the
diagnosis in upstream issue #7914 and closed PR #37807, but keeps J.A.G.O.D.A's own
...
**Fix**: The sync loop automatically retries every 5 seconds on error. Check the J.A.G.O.D.A logs for sync-related warnings. If the bot consistently falls behind, ensure your homeserver has adequate resources.

### Bot is offline

**Cause**: The J.A.G.O.D.A gateway isn't running, or it failed to connect.
...
- **Media support**: J.A.G.O.D.A can send and receive images, audio, video, and file attachments. Media is uploaded to your homeserver using the Matrix content repository API.
