...
title: "SMS (Twilio)"
description: "Set up J.A.G.O.D.A as an SMS chatbot via Twilio"
---

# SMS Setup (Twilio)

J.A.G.O.D.A connects to SMS through the [Twilio](https://www.twilio.com/) API. People text your Twilio phone number and get AI responses back — same conversational experience as Telegram or Discord, but over standard text messages.
...
If you're running J.A.G.O.D.A locally, use a tunnel to expose the webhook:
...
Text your Twilio number — J.A.G.O.D.A will respond via SMS.
...
J.A.G.O.D.A validates that inbound webhooks genuinely originate from Twilio by verifying the `X-Twilio-Signature` header (HMAC-SHA1). This prevents attackers from injecting forged messages.
...
3. Check J.A.G.O.D.A gateway logs for Twilio API errors
