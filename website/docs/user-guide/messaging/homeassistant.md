...
description: Control your smart home with J.A.G.O.D.A via Home Assistant integration.
...
J.A.G.O.D.A integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "J.A.G.O.D.A"
5. Copy the token
...
J.A.G.O.D.A registers four tools for smart home control:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "J.A.G.O.D.A".
...
from `config.yaml`. Double-check the file lives under the active J.A.G.O.D.A profile
...
running J.A.G.O.D.A — `curl -H "Authorization: Bearer <token>" <url>/api/` should
