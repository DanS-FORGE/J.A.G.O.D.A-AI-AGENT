...
messaging platform J.A.G.O.D.A is already configured for. Think of it as a
...
Any platform J.A.G.O.D.A ships adapters for works as a target:
...
When stdin is a TTY (no pipe), J.A.G.O.D.A does **not** wait for input — you'll
...
Because `hermes send` reuses your J.A.G.O.D.A config, the same script works on
any host where J.A.G.O.D.A is installed — no need to export bot tokens into
...
| Approach | Multi-platform | Reuses J.A.G.O.D.A creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
