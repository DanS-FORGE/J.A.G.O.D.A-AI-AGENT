...
description: "Diagnose and fix common J.A.G.O.D.A cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
The `script` path must be an absolute path (or relative to the J.A.G.O.D.A config directory). Verify:
```bash
...
hermes logs                         # View recent J.A.G.O.D.A logs
