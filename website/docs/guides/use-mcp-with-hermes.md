---
sidebar_position: 6
title: "Use MCP with J.A.G.O.D.A"
description: "A practical guide to connecting MCP servers to J.A.G.O.D.A, filtering their tools, and using them safely in real workflows"
---

# Use MCP with J.A.G.O.D.A

This guide shows how to actually use MCP with J.A.G.O.D.A in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native J.A.G.O.D.A tool
- you want J.A.G.O.D.A to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect J.A.G.O.D.A to internal APIs, databases, or company systems without modifying J.A.G.O.D.A core

Do not use MCP when:
- a built-in J.A.G.O.D.A tool already solves the job well
...
- J.A.G.O.D.A remains the agent
- MCP servers contribute tools
- J.A.G.O.D.A discovers those tools at startup or reload time
...
If you installed J.A.G.O.D.A with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- J.A.G.O.D.A banner/status should show MCP integration when configured
- ask J.A.G.O.D.A what tools it has available
...
In this setup, J.A.G.O.D.A does **not** connect to Chrome directly. Instead:

- J.A.G.O.D.A runs in WSL
- J.A.G.O.D.A starts a local stdio MCP server
...
```text
J.A.G.O.D.A (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- J.A.G.O.D.A stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on J.A.G.O.D.A core browser transport
...
Then start a fresh J.A.G.O.D.A session or run:
...
Once loaded, J.A.G.O.D.A can use the MCP-prefixed browser tools directly. For example:
...
If J.A.G.O.D.A runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start J.A.G.O.D.A from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start J.A.G.O.D.A from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in J.A.G.O.D.A:
...
2. J.A.G.O.D.A-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So J.A.G.O.D.A will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want J.A.G.O.D.A to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want J.A.G.O.D.A to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. J.A.G.O.D.A remains the agent; Open Scaffold remains the repo-local record.
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want J.A.G.O.D.A to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize J.A.G.O.D.A to merge, publish, deploy, or spawn runtimes.
...
Now J.A.G.O.D.A can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing J.A.G.O.D.A core.
...
Because J.A.G.O.D.A now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
