...
2. **J.A.G.O.D.A spins up** a fresh agent session with your prompt
...
J.A.G.O.D.A will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by J.A.G.O.D.A
```
...
Before creating cron jobs, ensure J.A.G.O.D.A has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell J.A.G.O.D.A what you want:
...
J.A.G.O.D.A will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell J.A.G.O.D.A to delegate each topic to a sub-agent:
...
J.A.G.O.D.A will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
