...
This page covers all commands related to [J.A.G.O.D.A profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
This shows the profile's J.A.G.O.D.A home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your J.A.G.O.D.A installation.
...
J.A.G.O.D.A version, author, env var requirements, the source URL/path, and
...
J.A.G.O.D.A version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host J.A.G.O.D.A can clone from.
...
Global flag to run any J.A.G.O.D.A command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
