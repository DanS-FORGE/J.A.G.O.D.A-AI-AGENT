...
description: "Authoritative reference for J.A.G.O.D.A terminal commands and command families"
---
...
| `--profile <name>`, `-p <name>` | Select which J.A.G.O.D.A profile to use for this invocation. Overrides the sticky default set by `hermes profile use`. |
...
| `hermes peer` | Register peer J.A.G.O.D.A gateways on other machines and DM their agents' canonical Bot Chats (`hermes peer dm <peer>[/<agent>] "…"`). The transport behind cross-machine bot-to-bot messaging. |
...
| `hermes backup` | Back up J.A.G.O.D.A home directory to a zip file. |
...
| `hermes import` | Restore a J.A.G.O.D.A backup from a zip file. |
| `hermes logs` | View, tail, and filter agent/gateway/error log files. |
| `hermes config` | Show, edit, migrate, and query configuration files. |
| `hermes skin` | List, switch, and tweak display skins. |
| `hermes console` | Open the safe J.A.G.O.D.A command console. |
...
| `hermes acp` | Run J.A.G.O.D.A as an ACP server for editor integration. |
| `hermes mcp` | Manage MCP server configurations and run J.A.G.O.D.A as an MCP server. |
| `hermes plugins` | Manage J.A.G.O.D.A plugins (install, enable, disable, remove). |
...
| `hermes serve` | Start the J.A.G.O.D.A backend server (headless; powers the desktop app and remote backends). |
| `hermes desktop` (alias `gui`) | Build and launch the native Electron desktop app. |
| `hermes profile` | Manage profiles — multiple isolated J.A.G.O.D.A instances. |
...
| `hermes uninstall` | Remove J.A.G.O.D.A from the system. |
...
| `--safe-mode` | Troubleshooting mode: disable ALL customizations — user config, rules/memory injection, plugins, shell hooks, and MCP servers (implies `--ignore-user-config` and `--ignore-rules`). Use to isolate whether a problem comes from your setup or from J.A.G.O.D.A itself. |
...
hermes chat --safe-mode -q "Is this bug mine or J.A.G.O.D.A's?"
```
...
Interactive provider + model selector. **This is the command for adding new providers, setting up API keys, and running OAuth flows.** Run it from your terminal — not from inside an active J.A.G.O.D.A chat session.
...
**`hermes model`** (run from your terminal, outside any J.A.G.O.D.A session) is the **full provider setup wizard**. It can add new providers, run OAuth flows, prompt for API keys, and configure endpoints.

**`/model`** (typed inside an active J.A.G.O.D.A chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys.

**If you need to add a new provider:** Exit your J.A.G.O.D.A session first (`Ctrl+C` or `/quit`), then run `hermes model` from your terminal prompt.
:::
...
| `enroll` | Experimental: enroll this gateway with a relay connector and save relay credentials for connector-backed platforms. See [J.A.G.O.D.A Relay](/user-guide/messaging/relay). |
...
| `--name NAME` | `J.A.G.O.D.A` | Bot display name in Slack. |
...
Bot-to-bot DMs across machines. Register another J.A.G.O.D.A gateway (any machine
...
> Not to be confused with `hermes claw migrate` (one-shot import of OpenClaw configuration into J.A.G.O.D.A) — `hermes migrate` is the top-level config-rewrite command.
...
On-demand vulnerability scan against [OSV.dev](https://osv.dev). Covers the J.A.G.O.D.A venv (installed PyPI distributions), Python dependencies declared by plugins under `~/.hermes/plugins/`, and pinned `npx`/`uvx` MCP servers in `config.yaml`. Does NOT scan globally-installed packages or editor/browser extensions.
...
| `--skip-venv` | off | Skip scanning the J.A.G.O.D.A Python venv. |
...
hermes auth spotify                                      # Authenticate J.A.G.O.D.A with Spotify via PKCE
```
...
Outputs a compact, plain-text summary of your entire J.A.G.O.D.A setup. Designed to be copy-pasted into Discord, GitHub issues, or Telegram when asking for support — no ANSI colors, no special formatting, just data.
...
| Section | Details |
|---------|---------|
| **Header** | J.A.G.O.D.A version, release date, git commit hash |
...
The report includes system info (OS, Python version, J.A.G.O.D.A version), recent agent, gateway, GUI/dashboard, and desktop logs (512 KB limit per file), and redacted API key status. By default, uploads are redacted so secrets are not included.
...
Create a zip archive of your J.A.G.O.D.A configuration, skills, sessions, and data. The backup excludes the hermes-agent codebase itself, and it does not nest earlier backup artifacts (`backups/`, `state-snapshots/`) — each of those already contains its own copy of `state.db`.
...
The backup uses SQLite's `backup()` API for safe copying, so it works correctly even when J.A.G.O.D.A is running (WAL-mode safe).
...
Restore a previously created J.A.G.O.D.A backup into your J.A.G.O.D.A home directory. All files in the archive overwrite existing files in your J.A.G.O.D.A home; `--force` only skips the confirmation prompt that fires when the target already has a J.A.G.O.D.A installation.
...
View, tail, and filter J.A.G.O.D.A log files. All logs are stored in `~/.hermes/logs/` (or `<profile>/logs/` for non-default profiles).
...
J.A.G.O.D.A uses Python's `RotatingFileHandler`. Old logs are rotated automatically — look for `agent.log.1`, `agent.log.2`, etc. The `hermes logs list` subcommand shows all log files including rotated ones.
...
- **Prompt tiers** — stable / context / volatile, matching how J.A.G.O.D.A layers
...
- `--source well-known` lets you point J.A.G.O.D.A at a site exposing `/.well-known/skills/index.json`.
...
`hermes moa configure` reuses J.A.G.O.D.A's provider → model picker for each reference model and the aggregator. A preset is an execution-mode configuration, not a primary model or provider.
...
Starts J.A.G.O.D.A as an ACP (Agent Client Protocol) stdio server for editor integration.
...
Manage MCP (Model Context Protocol) server configurations and run J.A.G.O.D.A as an MCP server.
...
| `serve [-v\|--verbose]` | Run J.A.G.O.D.A as an MCP server — expose conversations to other agents. |
...
See [MCP Config Reference](./mcp-config-reference.md), [Use MCP with J.A.G.O.D.A](../guides/use-mcp-with-hermes.md), and [MCP Server Mode](../user-guide/features/mcp.md#running-hermes-as-an-mcp-server).
...
See [Plugins](../user-guide/features/plugins.md) and [Build a J.A.G.O.D.A Plugin](../developer-guide/plugins/index.md).
...
If cua-driver is already present, J.A.G.O.D.A checks its version and runtime
...
installer. J.A.G.O.D.A never replaces a custom binary selected through
...
The built-in `computer_use` toolset is the recommended J.A.G.O.D.A integration.
Registering raw Cua MCP tools is an alternative when you need Cua's low-level
tool vocabulary. `cua-driver skills install` detects J.A.G.O.D.A and links Cua's
skill pack into the J.A.G.O.D.A skills directory automatically.

Permission mode and capability-manifest approval
belong to runtime launch. In bounded mode J.A.G.O.D.A passes Cua's canonical
...
right now without waiting for the next J.A.G.O.D.A update.
...
[Petdex](https://github.com/crafter-station/petdex) is a public gallery of animated sprite pets for coding agents. Install one and J.A.G.O.D.A shows it reacting to agent activity across the CLI, TUI, and desktop app.
...
Migrate your OpenClaw setup to J.A.G.O.D.A. Reads from `~/.openclaw` (or a custom path) and writes to `~/.hermes`. Automatically detects legacy directory names (`~/.clawdbot`, `~/.moltbot`) and config filenames (`clawdbot.json`, `moltbot.json`).

| Option | Description |
|--------|-------------|
| `--dry-run` | Preview what would be migrated without writing anything. |
| `--preset <name>` | Migration preset: `full` (all compatible settings) or `user-data` (excludes infrastructure config). Neither preset imports secrets — pass `--migrate-secrets` explicitly. |
| `--overwrite` | Overwrite existing J.A.G.O.D.A files on conflicts (default: refuse to apply when the plan has conflicts). |
...
The migration covers 30+ categories across persona, memory, skills, model providers, messaging platforms, agent behavior, session policies, MCP servers, TTS, and more. Items are either **directly imported** into J.A.G.O.D.A equivalents or **archived** for manual review.
...
Import a **Claude Code** (`~/.claude`) or **OpenAI Codex CLI** (`~/.codex`) setup into J.A.G.O.D.A. Maps `CLAUDE.md`/`AGENTS.md` instructions to memory entries, `Bash(...)` permission allow/deny rules to `command_allowlist`/`approvals.deny`, MCP servers to `mcp_servers` in `config.yaml`, and skill directories into `~/.hermes/skills/`. Always previews before applying; API keys and credentials are never imported.
...
Manage profiles — multiple isolated J.A.G.O.D.A instances, each with its own config, sessions, skills, and home directory.
...
Print a shell completion script to stdout. Source the output in your shell profile for tab-completion of J.A.G.O.D.A commands, subcommands, and profile names.
...
`hermes update` pulls the configured update branch (default: `main`). If your checkout is on another branch, J.A.G.O.D.A may check out the update branch before pulling. Commit branch work before updating when you want to keep it outside the update autostash flow.
...
- **Gateway restart.** After a successful update, J.A.G.O.D.A attempts to restart all running gateway profiles automatically so they pick up the new code. Use `hermes gateway restart` when you want to restart a gateway without applying an update.
...
- **npm lockfile churn.** Before stashing or switching branches, J.A.G.O.D.A makes a best-effort cleanup of tracked `package-lock.json` diffs produced by npm install/build steps. Commit or manually stash intentional lockfile edits before running `hermes update`.
...
- **Legacy `hermes.service` warning.** If J.A.G.O.D.A detects a pre-rename `hermes.service` systemd unit (instead of the current `hermes-gateway.service`), it prints a one-time migration hint so you can avoid flap-loop issues.
...
| `hermes uninstall [--full] [--gui] [--dry-run] [--yes]` | Remove J.A.G.O.D.A, optionally deleting all config/data. `--gui` removes only the desktop Chat GUI, leaving the agent intact; `--full` also deletes config/data; `--dry-run` prints what would be removed without changing anything; `--yes` skips prompts. |
