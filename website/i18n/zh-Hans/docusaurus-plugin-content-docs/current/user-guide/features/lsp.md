...
J.A.G.O.D.A 以后台子进程方式运行完整的语言服务器——pyright、gopls、rust-analyzer、
...
这与顶级编码 agent 所采用的架构相同。J.A.G.O.D.A 将其作为自包含组件提供：
...
J.A.G.O.D.A 会自动检测 PATH 上或 `<HERMES_HOME>/lsp/bin/` 中的二进制文件。
...
当 `install_strategy: auto` 时，J.A.G.O.D.A 将二进制文件安装到 `<HERMES_HOME>/lsp/bin/`。
...
J.A.G.O.D.A 管理，重置 profile 时会被删除。
...
服务器在 J.A.G.O.D.A 进程的整个生命周期内保持运行。没有空闲超时回收机制——每次写入都
