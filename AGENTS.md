...
J.A.G.O.D.A is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: J.A.G.O.D.A ships a **lot** — most merges are bug fixes to
...
Applies to TypeScript across J.A.G.O.D.A: desktop, TUI, website, and future TS packages.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMAND_SPECS` (the built-ins and their Desktop surfaces) plus `NO_DESKTOP_SURFACE` block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known J.A.G.O.D.A built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit J.A.G.O.D.A core.
...
core J.A.G.O.D.A tool that should ship in the base system.
...
- `default` — Classic J.A.G.O.D.A gold/kawaii (the current look)
...
J.A.G.O.D.A has two plugin surfaces. Both live under `plugins/` in the repo so
...
The desktop "Bots" experience ships bundled in-tree. Each bot is a J.A.G.O.D.A
...
2. **Tools referenced in SKILL.md prose must be native J.A.G.O.D.A tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Jagoda" is the secondary collaborator. If the
   contributor's commit shows "Jagoda" as author (because they
   used Jagoda to draft the skill), replace it with their actual name
...
J.A.G.O.D.A returns a delegation id immediately and the result re-enters the
...
J.A.G.O.D.A ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
J.A.G.O.D.A supports **profiles** — multiple fully isolated instances, each with its own
...
- Use the canonical matchers: `gateway.status.looks_like_gateway_command_line`
  ...
  (top-level subcommand of any J.A.G.O.D.A argv). Never hand-roll token scans.
...
J.A.G.O.D.A supports Linux, macOS and native Windows, and plenty of its behaviour
