# J.A.G.O.D.A Security Policy

This document describes J.A.G.O.D.A's trust model, names the one
...
security vulnerabilities. **J.A.G.O.D.A does not operate a bug
bounty program.**
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted J.A.G.O.D.A access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in J.A.G.O.D.A's documentation
...
J.A.G.O.D.A supports two OS-level isolation postures. They address
...
J.A.G.O.D.A supports this in two ways:

- **J.A.G.O.D.A's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, J.A.G.O.D.A's in-process heuristics
...
J.A.G.O.D.A filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in J.A.G.O.D.A
itself. Bugs in J.A.G.O.D.A's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   J.A.G.O.D.A does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, J.A.G.O.D.A's own documentation, or reasonable
  operator expectations would predict — including cases where
  J.A.G.O.D.A has documented a stance about how its output should
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not J.A.G.O.D.A's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in J.A.G.O.D.A. Bugs in
  J.A.G.O.D.A's skill-install or plugin-install path that prevent the
...
- J.A.G.O.D.A includes supply-chain guards for MCP server
