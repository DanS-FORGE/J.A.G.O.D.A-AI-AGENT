...
Small Node helper that bridges J.A.G.O.D.A to Photon's Spectrum SDK
(`spectrum-ts`).  J.A.G.O.D.A is Python; Photon has no public HTTP
...
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and J.A.G.O.D.A can receive inbound messages
...
The J.A.G.O.D.A plugin's `hermes photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. J.A.G.O.D.A is Python, so replies go through this sidecar
