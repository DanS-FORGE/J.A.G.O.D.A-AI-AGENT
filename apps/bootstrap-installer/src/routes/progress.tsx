...
export default function ProgressScreen({ bootstrap }: ProgressProps) {
  ...
  const title = bootstrap.status === 'completed' ? 'Done' : isUpdate ? 'Updating J.A.G.O.D.A' : 'Setting up J.A.G.O.D.A'

  const description = isUpdate
    ? 'J.A.G.O.D.A is updating to the latest version — this only takes a moment.'
    : 'This is a one-time setup. The J.A.G.O.D.A installer is downloading dependencies and configuring your machine. Subsequent launches will skip this step.'
